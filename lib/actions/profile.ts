...
export async function updateCompanyName(company_name: string) {
  const parsed = companySchema.safeParse({ company_name });
  if (!parsed.success) return { error: "validation" as const };
  ...
  const value = parsed.data.company_name;
  const normalized = value && value.length > 0 ? value : null;

  const { error } = await supabase
    .from("profiles")
    .update({ company_name: normalized })
    .eq("id", user.id);
