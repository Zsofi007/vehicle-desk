...
export default async function DemoVehiclesPage({ params }: Props) {
  ...
  for (const a of alerts) {
    const cur = vehicleStatus.get(a.vehicle.id);
    ...
    else if (!cur) vehicleStatus.set(a.vehicle.id, "ok");
  }
  ...
  const rows: VehicleTileRow[] = vehicles.map((v) => {
    ...
    return {
      ...
      odometerOutdated: staleIds.has(v.id),
    };
  });
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-xl font-semibold text-slate-900 md:text-2xl">{tNav("vehicles")}</h1>
        <p className="mt-2 text-base text-slate-600">{tdemo("vehiclesPageSubtitle")}</p>
      </div>
        ...
        <div className="-mx-4 overflow-x-auto overscroll-x-contain px-4 md:mx-0 md:overflow-visible md:px-0">
          <div className="grid grid-flow-col auto-cols-[7rem] gap-3 md:grid-flow-row md:auto-cols-auto md:grid-cols-3 md:gap-4">
            ...
            <div className="aspect-square rounded-xl border border-slate-200 bg-white p-3 shadow-sm md:aspect-auto md:p-4">
              ...
              <div className="mt-2 text-xs text-slate-600 md:text-sm">{tStatus("badgeExpired")}</div>
            </div>
          </div>
        </div>
      </section>
