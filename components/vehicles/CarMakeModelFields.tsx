...
export function CarMakeModelFields({
  ...
  const knownModel = initialModel ? modelOptions.some((o) => o.value === initialModel) : false;
  const [model, setModel] = useState(knownModel ? initialModel : "");
  const [customModel, setCustomModel] = useState(knownModel ? "" : initialModel);
  const [modelIsCustom, setModelIsCustom] = useState(!knownModel && initialModel.length > 0);
